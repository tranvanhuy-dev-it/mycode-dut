...
// Hàm tính xác suất hôm nay ăn 'Pho' biết hôm qua đã ăn 'Banh my'
double probabilityOfPhoAfterBanhMy(const vector<vector<double>>& transition_matrix, const map<string, int>& food_to_index, const vector<string>& order) {
    // Xác định chỉ số của 'Banh my' và 'Pho' trong ma trận trạng thái
    int index_banh_my = food_to_index.at("Banh my");
    int index_pho = food_to_index.at("Pho");

    return transition_matrix[index_banh_my][index_pho];
}

// Hàm in ma trận trạng thái
void printTransitionMatrix(const vector<vector<double>>& matrix, const map<string, int>& food_to_index, const vector<string>& order) {
...
int main() {
    ...
    cout << "Xac xuat hom nay an Pho biet hom qua da an Banh My: " << probabilityOfPhoAfterBanhMy(transition_matrix, food_to_index, order);
